...
typedef void (*led_switch_on_func_t)(void);
typedef void (*led_switch_off_func_t)(void);
led_switch_on_func_t led_on_funcs[] = {
    led0_switch_on,
    led1_switch_on,
    led2_switch_on,
};
led_switch_off_func_t led_off_funcs[] = {
    led0_switch_off,
    led1_switch_off,
    led2_switch_off,
};
void *app_led_malloc(size_t size)
{
    return Mem_malloc(INSRAM, size);
}
void app_led_free(void *use)
{
    Mem_free(INSRAM, use);
}

// 定义信号灯对象句柄
led_t *leds[LED_NUM] = {NULL};
// 定义内存操作函数接口
led_mem_opreation_t app_led_mem_opreation;

void blink_over_callback(led_t *led_handler)
{
    led_set_mode(led_handler, LOOP_PERMANENT, led_blink_mode_5);
    led_start(led_handler);
}

static MultiTimer leds_timer;
static void leds_ticks_callback(MultiTimer *timer, void *userData)
{
    led_ticks();
    multiTimerStart(&leds_timer, LED_TICK_TIME, leds_ticks_callback, NULL);
}
...
void app_led_init(void)
{
    ...
    app_led_mem_opreation.malloc_fn = app_led_malloc;
    app_led_mem_opreation.free_fn = app_led_free;
    led_set_mem_operation(&app_led_mem_opreation);

    // 初始化信号灯对象
    for (uint8_t i = 0; i < LED_NUM; i++)
    {
        leds[i] = led_create(led_on_funcs[i], led_off_funcs[i]);
        // 设置信号灯工作模式，循环十次
        led_set_mode(leds[i], 10, led_blink_mode_0);
        // 设置信号灯闪烁结束回调函数
        led_set_blink_over_callback(leds[i], blink_over_callback);
        // 开启信号灯
        led_start(leds[i]);
    }
}

...
/* 如果没有使用 Zephyr，定义一些必要的工具宏 */
/* Keil AC5 兼容的编译时断言 */
#ifndef BUILD_ASSERT
#define BUILD_ASSERT(expr, msg) typedef char static_assert_##__LINE__[(expr) ? 1 : -1]
#endif

/* BIT(x) - 生成第 x 位为 1 的值，例如 BIT(0) = 1, BIT(3) = 8 */
#ifndef BIT
#define BIT(x) (1U << (x))
#endif

/* DIV_ROUND_UP(a, b) - 向上取整除法，例如 DIV_ROUND_UP(5, 3) = 2 */
#ifndef DIV_ROUND_UP
#define DIV_ROUND_UP(a, b) (((a) + (b) - 1) / (b))
#endif

/* __ASSERT - 运行时断言，Keil AC5 可简化为空或使用 assert() */
#ifndef __ASSERT
    #include <assert.h>
    #define __ASSERT(expr, msg) assert(expr)
#endif

/* __ASSERT_NO_MSG - 无消息的断言，简化为空 */
				 ...
				 uint8_t *data)
{
	...
	if (size > RING_BUFFER_MAX_SIZE) {
		return;  /* 严重错误应该使用断言，但这里用返回避免编译问题 */
	}
...
static inline void ring_buf_item_init(struct ring_buf *buf,
				      ...
				      uint32_t *data)
{
	if (size > RING_BUFFER_MAX_SIZE / 4) {
		return;
	}
 ...
 */
static inline uint8_t ring_buf_is_empty(const struct ring_buf *buf)
{
